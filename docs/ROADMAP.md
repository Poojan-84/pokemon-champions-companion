...
- **Archetype vs. Archetype Matchup Pages v1 (fifth Phase 2 milestone shipped)**: /matchups (index) and /matchups/[slug] (6 detail pages) — every archetype-vs-archetype pairing among the four current Team Archetypes guide entries (Sun Offense, Rain, Sand Offense, Trick Room), each with a verdict (A/B/even), 2-4 key factors grounded in real `lib/typeChart.ts` effectiveness math and each archetype's actual representative Pokémon, and cross-links to `/guides/team-archetypes` and relevant Rental Team entries. Ships archetype-vs-archetype only (not Pokémon-vs-Pokémon or core-vs-core), and every entry carries `"confidence": "draft"` with a prominent "pending founder review" banner on every page — see DECISION_LOG.md for the scope call and why draft status is load-bearing here, plus **the founder needs to review the 6 verdicts and their key factors before this is treated as real competitive guidance** (flagged in the PR/session notes, not something code review alone can validate). NavBar and homepage feature grid updated — Matchups now live, not "Coming soon"
- **Guide page visual polish (sixth Phase 2 milestone shipped)**: an audit of all 3 guides' live rendered output found callout boxes and section-heading iconography were already at 100% coverage across all 3 (built incrementally as guides shipped, never tracked as their own milestone) — no work needed there. The one genuinely missing, consistently-absent piece was a pull-quote/highlighted-stat treatment for the single most decision-relevant fact per guide; added a new `.guide-highlight` style (`app/globals.css`) and applied it once per guide, each quoting existing guide prose verbatim (no copy changes) — the doubles-format fact (Beginner Overview), the 22-Pokémon/16-Mega-Evolution stat (Regulation M-B), and the "pick one weather archetype" recommendation (Team Archetypes). Reuses only already contrast-verified color tokens; Lighthouse unchanged (97/100/100/100) across all 3 guides.

### Current 🚧
(none — Guide page visual polish just shipped; next Phase 2 item not yet chosen)

### Next ⏭️
- Phase 2 (Tools), remaining and unordered until we decide on sequencing: visual identity pass, per-Pokémon meta builds
...
- ~~Matchup pages~~ — v1 shipped (archetype-vs-archetype, all 6 pairings), see Completed above. Pokémon-vs-Pokémon and core-vs-core matchup granularity deferred — see DECISION_LOG.md
- ~~Speed calculator~~ — shipped, see Completed above
- ~~Guide page visual polish~~ — shipped, see Completed above. Callout boxes and section iconography were already fully in place; added the missing pull-quote/highlighted-stat treatment
